...
export const scoreBeet = (beet: BeetScoreInput): number => {
  ...
  const pairPoints = Math.min(Math.max(0, beet.tomatoes), Math.max(0, beet.peppers))
...
export const calculateRoundTotals = (players: Player[], roundScores: RoundScore[]): PlayerRoundTotal[] => {
  const beetTotals = roundScores.map(scoreRoundBeets)
  const bonusPoints = calculateBonusPoints(beetTotals)

  return players.map((player, index) => {
    const animalPoints = Math.max(0, roundScores[index]?.animalCardsFulfilled ?? 0) * 5
...
export const calculateGameTotals = (game: Game): Record<string, number> =>
  Object.fromEntries(
    game.players.map((player) => {
      const total = game.rounds[player.id].slice(0, game.currentRound).reduce((sum, round, index) => {
        const roundScores = game.players.map((candidate) => game.rounds[candidate.id][index])
        const roundTotal = calculateRoundTotals(game.players, roundScores).find((entry) => entry.playerId === player.id)
